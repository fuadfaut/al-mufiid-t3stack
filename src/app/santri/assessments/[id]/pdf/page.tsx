...
export default function SantriAssessmentPDFPage({ params }: { params: { id: string } }) {
  ...
  const generatePDF = () => {
    ...
    try {
      ...
      if (assessment.fashahahAssessment) {
        doc.text("Penilaian Fashahah:", 14, 85);
      ...
      doc.text(`(${assessment.createdBy.name || ""})`, 140, signatureY + 30);
      
      // Save the PDF
      doc.save(`Penilaian_${session?.user?.name}_${new Date().toISOString().split("T")[0]}.pdf`);
    } catch (error) {
    ...
    <SantriLayout>
      <div className="container mx-auto px-4 py-8">
        <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
          ...
          <button
            onClick={() => router.push(`/santri/assessments/${params.id}`)}
          ...
          <div className="rounded-lg bg-white p-6 shadow-md">
            <div className="text-center">
              {pdfGenerated ? (
                ...
                <>
                  <div className="mb-4 h-12 w-12 animate-spin rounded-full border-b-2 border-t-2 border-purple-500 mx-auto"></div>
