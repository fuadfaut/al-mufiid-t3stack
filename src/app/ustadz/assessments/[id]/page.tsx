...
export default function AssessmentDetailPage({ params }: { params: { id: string } }) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      router.push("/ustadz/assessments");
    } catch (error) {
  ...
  const handleExportPDF = () => {
    router.push(`/ustadz/assessments/${params.id}/pdf`);
  };
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
          ...
          <div className="lg:col-span-1">
            <div className="rounded-lg bg-white p-6 shadow-md">
              <h2 className="mb-4 text-lg font-medium text-gray-800">Informasi Santri</h2>
              <div className="space-y-3">
                <div>
                  <p className="text-sm font-medium text-gray-500">Nama</p>
                  <p className="text-gray-800">{assessment.student.name}</p>
                </div>
                {assessment.student.student && (
                  <>
                    <div>
                      <p className="text-sm font-medium text-gray-500">NIS</p>
                      <p className="text-gray-800">{assessment.student.student.nis}</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-gray-500">Kelas</p>
                      <p className="text-gray-800">{assessment.student.student.class}</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-gray-500">Jilid</p>
                      <p className="text-gray-800">{assessment.student.student.jilid}</p>
                    </div>
                  </>
                )}
              </div>

              <h2 className="mb-4 mt-6 text-lg font-medium text-gray-800">Informasi Penilaian</h2>
              <div className="space-y-3">
                <div>
